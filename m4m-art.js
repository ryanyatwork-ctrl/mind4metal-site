(function () {
  ...
  function trimResolveCache(cacheObj) {
    ...
    const trimmed = {};
    entries.slice(entries.length - RESOLVE_CACHE_LIMIT).forEach(([k, v]) => {
      trimmed[k] = v;
    });
    return trimmed;
  }
  ...
  async function loadManifest() {
    ...
    const canUseCached = state.config.cacheManifest && cached && cached.savedAt && (now - cached.savedAt) < MANIFEST_TTL_MS && cached.data;
  ...
  async function init(config) {
    ...
    if (!state.initPromise) {
      state.initPromise = loadManifest().catch((error) => {
  ...
  function resolveFromManifest(artist, title, album) {
    ...
    if (directTrack?.art) {
      return {
        url: absolutizeAssetPath(directTrack.art),
        source: 'manifest-track',
        entry: directTrack,
      };
    }
    ...
    if (album) {
      ...
      if (albumEntry?.art) {
        return {
          url: absolutizeAssetPath(albumEntry.art),
          source: 'manifest-album',
          entry: albumEntry,
        };
      }
    }
    ...
    if (artistEntry?.art) {
      return {
        url: absolutizeAssetPath(artistEntry.art),
        source: 'manifest-artist',
        entry: artistEntry,
      };
    }
  ...
  function parseTrack(raw) {
    ...
    if (pieces.length >= 2) {
      return {
        ...
        title: pieces.slice(1).join(' - ').trim(),
        raw: String(raw),
      };
    }
  ...
  async function primeCache(entries) {
    ...
    for (const entry of entries) {
      const resolved = await resolveArt(entry || {});
